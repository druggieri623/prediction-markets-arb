        ...
        if args.json:
            ...
            for pair in pairs:
                result.append({
                    ...
                    "classifier_probability": (
                        ...
                        if pair.classifier_probability else None
                    ),
                    "name_similarity": (
                        round(pair.name_similarity, 4)
                        if pair.name_similarity else None
                    ),
                    "category_similarity": (
                        round(pair.category_similarity, 4)
                        if pair.category_similarity else None
                    ),
                    "temporal_proximity": (
                        round(pair.temporal_proximity, 4)
                        if pair.temporal_proximity else None
                    ),
            ...
            if not pairs:
                ...
                for i, pair in enumerate(pairs, 1):
                    confirmed_badge = "✓ CONFIRMED" if pair.is_manual_confirmed else "◯ unconfirmed"
                    ...
                    if pair.classifier_probability is not None:
                        print(f"    Classifier prob:    {pair.classifier_probability:6.2%}")
                    ...
                    if pair.category_similarity is not None:
                        print(f"    Category similar:   {pair.category_similarity:6.2%}")
