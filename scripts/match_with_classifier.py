...
def main():
    ...
    try:
        ...
        agi_markets = [m for m in markets if "agi" in m.name.lower() or "artificial general" in m.name.lower()]
            ...
            for market_b in markets[i + 1:]:
                if market_a.source != market_b.source:
                    ...
                    if prob >= args.threshold:
                        ...
                        print(f"  Source 1: {market_a.source:12s} | {market_a.name[:50]}")
                        print(f"  Source 2: {market_b.source:12s} | {market_b.name[:50]}")
                        print(f"  ")
                        print(f"  Classifier Probability: {prob:6.2%}")
                        print(f"  Rule-based Score:       {match_result.match_score:6.2%}")
                        print(f"    - Name similarity:     {match_result.name_similarity:6.2%}")
                        print(f"    - Category similarity: {match_result.category_similarity:6.2%}")
                        print(f"    - Temporal similarity: {match_result.temporal_proximity:6.2%}")
                        print(f"  ")
